...
**画像は正しい向きで登録してください。登録時の向きが正解向きとなります。**  
出題時にはアプリがランダムに 0°・90°・180°・270° に回転して表示します。  
採点時には、最終的に 0° に戻されていれば向き正解と判定します。

`correctRotation` は常に `0` を設定してください。  
...
RadiographImage(
    ...
    correctRotation: 0    // 常に 0 — 画像を正しい向きで登録することが前提
)
```
...
**`correctRotation` は常に `0` です。**  
画像は正しい向きで `Assets.xcassets` に登録してください。

| タイミング | 動作 |
|---|---|
| 練習開始時 | 各画像をランダムに 0°・90°・180°・270° で表示 |
| 練習中 | 学生は回転ボタンで正しい向き（0°）に戻す |
| 採点時 | `currentRotation == 0` → 向き正解 / それ以外 → 向き不正解 |

新しい画像を追加する際は、正しい向きで登録するだけで設定完了です。  
`correctRotation` の値を変更する必要はありません。
